...
All three sources are city-wide; the map is not. `loadAll()` filters
everything down to the `SERVED_AREAS` listed in `js/data.js` — currently
`West End` and `Downtown` — after the merge, so one rule covers the
snapshot, the live API and the CSV alike.

The snapshot is left intact on disk rather than trimmed. It is a City-wide
export with WESN's `Domain` / `Subdomain` / `Amenity` columns applied on top,
and its area mix is wider than the catchment:
...
Two things to know before changing the gate. Areas are compared as whole
strings, because `DowntownEastside` is a distinct neighbourhood that a
substring test would pull in under `Downtown`. And records the City leaves
un-areaed are dropped — the sidebar already excluded them whenever a
neighbourhood was ticked, which it is by default.
